...
export const WorkBackground: React.FC = ()=> (
    <div className='grid grid-cols-1 lg:grid-cols-2 w-full min-h-screen top-0 sticky' >
        <div className='bg-black h-[30vh] lg:min-h-screen'></div>
        </div>
)
...
export const WorkLeft: React.FC<{progress: number}> = ({children, progress}) => {
    ...
    //{
        return (
            <div className='flex flex-col items-center justify-center text-3xl lg:text-3xl h-[30vh] lg:h-auto font-nunito' style={{
...
export const WorkRight: React.FC<{progress: number}> = ({children, progress}) => {
   ...
   // if(progress > 0.85) translateY = Math.max(-50, -(progress -0.85) * 2 * 50)
    //{
        return (
            <div className='flex flex-1 lg:items-center justify-center h-screen font-nunito' style={{
                transform: `translateY(${translateY}px)`
            }}>
